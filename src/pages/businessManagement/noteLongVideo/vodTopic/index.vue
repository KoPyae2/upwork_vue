  ...
  >
    <div class="table">
      <el-button
        ...
        v-permission:longvideo:topic:add
      ...
      <el-table
        ...
        :max-height="825"
      >
        <el-table-column prop="topic_id" label="ID" align="center" />
        <el-table-column
          prop="is_sort"
        ...
        >
          <template #default="scope">
            {{ findName(scope.row.topic_name) }}
          </template>
        </el-table-column>
        <el-table-column :label="t('noteLongVideo.picture')" align="center">
          <template #default="scope">
            <photo-provider v-if="scope.row.topic_img">
              <photo-consumer
                :intro="scope.row.topic_img"
                :key="scope.row.topic_img"
                :src="scope.row.topic_img"
              >
                <img
                  :src="scope.row.topic_img"
          ...
          <template #default="scope">
            ...
            >
              <el-button
                ...
                v-permission:longvideo:topic:edit
              >
...
export default {
  ...
  setup() {
    ...
    const getData = () => {
      ...
      http.businessManagement.getNoteLongVideoTopic(state.param).then((res) => {
        if (res.data.err_code == 0) {
          state.isLoading = false;
          state.total = res.data.data.total;
          state.tableList = res.data.data.video_topics;
          state.tableList.sort((a, b) => {
            return a.is_sort - b.is_sort;
          });
        }
      });
    };
      ...
      ElMessageBox.confirm(t("common.wantToChange"), t("common.warning"), {
        ...
        .then(() => {
          ...
          http.businessManagement
            .updateNoteLongVideoTopic(param)
            .then((res) => {
              if (res.data.err_code == 0) {
