<template>
  ...
  >
    <div class="form" v-permission:feedback:playback:list>
          ...
          >
            <el-input
              :placeholder="
                t(`feedback.feedback_content`) + ' , ' + t(`feedback.user_id`)
              "
              v-model="filterForm.uid_or_content"
              maxlength="25"
              show-word-limit
            />
          </el-form-item>
    ...
    <div class="table">
      <el-button
        ...
        v-permission:feedback:playback:list
      ...
      >
        ...
        <el-table-column :label="t(`feedback.typeof`)" align="center">
          {{ t("feedback.playback") }}
        </el-table-column>
        ...
        <el-table-column
          :label="t(`feedback.feedback_content`)"
          align="center"
          width="350px"
        >
          <template #default="scope">{{
            scope.row.feed_content ? scope.row.feed_content : "--"
          }}</template>
        </el-table-column>
        ...
        >
          <template #default="scope">
            {{ scope.row.created_at_str ? scope.row.created_at_str : "--" }}
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  setup() {
    ...
    const getData = () => {
      ...
      http.feedbackManagement.getPlayBackFeedback(state.param).then((res) => {
    ...
    const filter = () => {
      ...
      if (state.filterForm.uid_or_content) {
        state.param.uid_or_content = state.filterForm.uid_or_content;
      }
      ...
      data.forEach(
        ({
          ...
          exportData.push({
            ...
            user_id,
            user_name,
            typeof: t(`feedback.playback`),
            reason_title: reason_title ? reason_title : "--",
            feed_content: feed_content
              ? feed_content.replace(/(\r\n|\n|\r)/gm, "")
              : "--",
            images,
            created_at_str,
          });
    ...
    const excelExport = () => {
      ...
      import("@/utils/Export2Excel").then((excel) => {
        ...
        excel.export_json_to_excel({
          header: tHeader,
          data,
          filename: t("menu.playback_feedback") + date,
          autoWidth: true,
          bookType: "xlsx",
        });
        state.isLoading = false;
