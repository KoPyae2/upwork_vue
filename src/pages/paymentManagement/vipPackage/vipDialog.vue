...
export default {
  ...
  setup(props, context) {
        ...
        if (valid) {
          if (state.dialogTitle == t("common.addTo")) {
            ...
            http.paymentManagement
              ...
              .catch((e) => {
                //  console.log(e);
                ElMessage.error(e.response.data.err_msg);
              });
          } else {
            state.doubleClick = true;
            http.paymentManagement
              .updateVipPackage(state.form)
              .then((res) => {
                if (res.data.err_code == 0) {
                  state.doubleClick = false;
                  closeDialog(formRef);
    ...
    const openDialog = () => {
      ...
      state.form.is_svip = 1;
      state.form.vip_type = props.data.vip_type;
      // state.form.vip_type_name = props.data.vip_type_name;
