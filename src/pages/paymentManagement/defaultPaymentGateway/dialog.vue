    ...
    <el-form label-width="150px" ref="formRef" :model="form">
      ...
      >
        ...
        >
          ...
          <el-form-item
            :label="t('defaultPayment.dialog.name')"
      ...
      <el-form-item
        :label="t('defaultPayment.dialog.icon')"
        prop="payicon"
        :rules="[
          { required: true, message: t('error.required'), trigger: 'blur' },
        ]"
      ...
      <el-form-item :label="t('defaultPayment.dialog.default')">
        ...
        if (valid) {
          if (state.dialogTitle == t("common.addTo")) {
            ...
            http.paymentManagement
              .createPaymentGateway(state.form)
              .then((res) => {
            ...
            state.doubleClick = true;
            http.paymentManagement
              .updatePaymentGateway(state.form)
              .then((res) => {
                if (res.data.err_code == 0) {
                  state.doubleClick = false;
                  closeDialog(formRef);
